...
func main() {
	...
	api.HandleFunc("POST /account/{id}/backfill", handler.RequireRole(keycloakProvider, keycloak.RoleManageBankAccounts, handler.BackfillAccount(pool, appConfig.FioAPIURL, appConfig.BankTokenEncryptionKey, appConfig.DisableFioSync, appConfig.Debug)))
